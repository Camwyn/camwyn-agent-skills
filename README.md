...
│   └── cc-post-bash.ps1           # PostToolUse:Bash: queue a milestone after `git commit`
│
├── scripts/                       # Sync engines invoked by the CLI, hooks, and git hooks
│
└── skills/
    ├── audit-skills/              # IDE-wide dual-pass telemetry & skill auditor
    │   └── SKILL.md
    │
    ├── wrap/                      # End-of-day close-out; extends handoff, distills to vault
    │   └── SKILL.md
    │
...
## 🌙 Session Lifecycle (`skills/wrap/`)

| Skill | Trigger / Command | Description |
|---|---|---|
| **[`wrap`](skills/wrap/SKILL.md)** | `/wrap`, "wrap up", "end of day" | End-of-day close-out. Extends `handoff`: snapshots repo state, distills `.ai/` + `.scratch/` outcomes into the vault `Overview`/`Tasks`/`Worklog`/`Decisions`, drains the live-sync queue, writes a rolling `_Resume.md`, and reports where things stand. Never commits, never pushes. Honors the **Record Ownership** boundary in `rules/obsidian-live-sync.md`. |
...
### 🪝 Claude Code Hook Adapters (`hooks/`)

`install` copies two adapters into `~/.agents/hooks/`. They do nothing until you reference them from `~/.claude/settings.json`:

| Adapter | Event | What it does |
|---|---|---|
| `cc-post-bash.ps1` | `PostToolUse` (matcher `Bash`) | After a real `git commit`, runs `obsidian-post-commit.ps1` to queue the milestone. Non-blocking. |
| `cc-session-start.ps1` | `SessionStart` | Auto-drains a pending `.agents/pending-sync.json` into the vault Worklog (unless `auto_sync.flush_on_session_start` is `false`), and — only if `site_audit.sites` is configured — flags stale site-health reports. Prints nothing when all is current. |

```jsonc
// ~/.claude/settings.json
"hooks": {
  "SessionStart": [ { "hooks": [ { "type": "command",
    "command": "powershell -NoProfile -ExecutionPolicy Bypass -File \"C:/Users/<you>/.agents/hooks/cc-session-start.ps1\"" } ] } ],
  "PostToolUse": [ { "matcher": "Bash", "hooks": [ { "type": "command",
    "command": "powershell -NoProfile -ExecutionPolicy Bypass -File \"C:/Users/<you>/.agents/hooks/cc-post-bash.ps1\"" } ] } ]
}
```

**Config keys these rely on** (`~/.agents/obsidian-config.json`):

| Key | Purpose |
|---|---|
| `vault_path` | Absolute path to the Obsidian vault on disk. Also settable via `$env:OBSIDIAN_VAULT_PATH`. Required for any filesystem sync (CLI, hooks, audit, digest). |
| `project_aliases` | `{ "<repo folder name>": "<vault project folder>" }` — for nested repos, monorepo subprojects, or renamed repos so commits log to the right project. |
| `auto_sync.flush_on_session_start` | `false` disables the SessionStart auto-drain. |
| `site_audit` | `{ "reports_dir", "cadence_days", "sites": { "<domain>": "<report slug>" } }` — opt-in staleness reminders. Empty `sites` = feature off. |
